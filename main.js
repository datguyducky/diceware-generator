function diceWare(){
    ...
    for(i=1;i<=numPhrase;i++){ // loop for generating random set of number (from 1 to 6). Length of loop depends on user input.
        ...
        var result = Math.floor(Math.random() * 6) + 1; // generating 5 times random number (from 1 to 6) which will get us something like for ex. 11111 - and this is gonna used to get world from bealeWordList
        result.toString;
        cont += result;
        }
